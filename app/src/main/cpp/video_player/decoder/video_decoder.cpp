...
void VideoDecoder::startUploader(UploaderCallback *pUploaderCallback) {
    ...
    textureFrameUploader->registerUpdateTexImageCallback(update_tex_image_callback,
...
int
VideoDecoder::initAnalyzeDurationAndProbesize(int *max_analyze_durations, int analyzeDurationSize,
...
//	int_cb = {VideoDecoder::interrupt_cb, this};
    ...
    if ((openInputErrCode = this->openFormatInput(videoSourceURI)) != 0) {
        LOGI("Video decoder open input file failed... videoSourceURI is %s openInputErr is %s",
             videoSourceURI, av_err2str(openInputErrCode));
        return -1;
    }
...
//	LOGI("video disposition %d", videoStream->disposition);
    LOGI("videoCodecCtx->pix_fmt is %d {%d, %d}", videoCodecCtx->pix_fmt, AV_PIX_FMT_YUV420P,
         AV_PIX_FMT_YUVJ420P);
...
//	LOGI("audioCodec->name is %s audioCodec->id is %d AV_CODEC_ID_PCM_S16LE is %d", audioCodec->name, audioCodec->id, AV_CODEC_ID_PCM_S16LE);
    ...
    if (!audioCodecIsSupported(audioCodecCtx)) {
        ...
        swrContext = swr_alloc_set_opts(NULL,
                                        ...
                                        AV_SAMPLE_FMT_S16, audioCodecCtx->sample_rate,
                                        av_get_default_channel_layout(audioCodecCtx->channels),
...
void VideoDecoder::avStreamFPSTimeBase(AVStream *st, float defaultTimeBase, float *pFPS,
                                       float *pTimeBase) {
    float fps, timebase;
...
bool VideoDecoder::decodeAudioFrames(AVPacket *packet, std::list<MovieFrame *> *result,
                                     float &decodedDuration,
...
//                LOGI("DEBUG I = %d", i);
    ...
    int actualSize = -1;
    if (mUploaderCallback)
        actualSize = mUploaderCallback->processAudioData((short *) audioData, numElements, position,
                                                         &buffer);
    else
        LOGE("VideoDecoder::mUploaderCallback is NULL");
    if (actualSize < 0) {
